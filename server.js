 ...
 * NewsData.io (free) for news + OpenAI GPT-4o-mini for summarisation
 */
...
// ─── Fetch UAE news from NewsData.io ──────────────────────────────────────
async function fetchUAENews() {
  ...
  if (!res.ok || data.status !== "success") throw new Error(data.message || "NewsData.io error");
...
// ─── Summarise with OpenAI GPT-4o-mini ────────────────────────────────────
async function summariseWithOpenAI(articles) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are UAEBrief, an AI agent producing a concise daily UAE news digest. Always respond with valid JSON only, no markdown, no extra text.",
        },
        {
          role: "user",
          content: `Today is ${today}. Below are recent UAE news articles from public sources.

Return ONLY a valid JSON object with this exact structure:
{
  "date": "${today}",
  "stories": [
    { "emoji": "📈", "title": "Short headline", "summary": "One sentence under 25 words.", "source": "Source Name" }
  ]
}

Pick exactly 5 stories. Focus on UAE business, economy, government, real estate, tech.
...
async function runAgent() {
  ...
  if (!articles.length) { console.error("❌ No articles"); return; }
  console.log("🧠 Summarising with OpenAI GPT-4o-mini...");
  const digest = await summariseWithOpenAI(articles);
...
<div class="bar">📰 NewsData.io · OpenAI GPT-4o-mini · UAE public news</div>
...
async function run(btn) {
  ...
  const t=setInterval(async()=>{
    ...
    try {
      ...
      else if(tries>20){clearInterval(t);c.innerHTML='<div class="error">⚠️ Check Render logs for errors.</div>';}
    }catch(e){}
  },3000);
}
load();
