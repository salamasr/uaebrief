 ...
 * Uses NewsData.io for news + Gemini for summarisation
 */
...
async function getWorkingModel() {
  ...
  for (const model of models) {
    try {
      ...
      console.log(`⚠️  ${model}: ${data.error?.message?.slice(0, 60)}`);
    } catch (e) {
...
async function summariseWithGemini(articles) {
  if (!GEMINI_MODEL) GEMINI_MODEL = await getWorkingModel();
...
Pick exactly 5 stories. Focus on business, economy, government, real estate, tech.
Articles:
${articleText}`;
...
<div class="bar">📰 NewsData.io · Gemini AI · UAE public news</div>
...
async function run(btn) {
  ...
  c.innerHTML='<div class="loading"><div class="spin"></div>Fetching UAE news and summarising…<br><small style="margin-top:8px;display:block">Takes about 15–20 seconds</small></div>';
