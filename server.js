 ...
 * NewsData.io (free) for news + Anthropic Claude for summarisation
 */
...
// ─── Summarise with Claude Haiku (cheapest, ~$0.001/day) ──────────────────
async function summarise(articles) {
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: `You are UAEBrief, an AI agent producing a concise daily UAE news digest.
...
async function runAgent() {
  ...
  console.log("🧠 Summarising with Claude Haiku...");
  const digest = await summarise(articles);
...
<div class="bar">📰 NewsData.io · Claude Haiku · UAE public news</div>
